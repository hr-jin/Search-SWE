...
3. **Executable and output validity.** Both entry points must exist and be executable, run read-only and across repeated requests, and honour the internal interfaces above. Retrieval has a combined 150-second budget for a full question set and answering a combined 1,800-second budget, with at most two answering API calls per question and at most 2,000 output tokens per call. Invalid output or a failed executable gate receives a score of `0`.
